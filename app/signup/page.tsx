  ...
  const handleRegister = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch('/api/auth/register', {
        method: 'POST',
        body: JSON.stringify({
          email: formData.get('Email Address'), // Matches label in map
          username: formData.get('Username'),
          firstName: formData.get('First Name'),
          lastName: formData.get('Last Name'),
          password: "TemporaryPassword123!", // Recommended: Add password input to UI
        }),
      });
      ...
      <div className="flex-grow flex flex-col md:flex-row bg-[#F6D8AB] min-h-[calc(100vh-80px)] overflow-hidden">
        ...
        >
          <div className="w-full">
            <div className="mb-10">
              <h1 className="font-lora text-4xl md:text-5xl text-[#280F0B] font-medium mb-4">Register with us.</h1>
            ...
            <form className="mt-8 space-y-8" onSubmit={handleRegister}>
                ...
                {[
                  ...
                  <div key={field.label} className="relative group lg:w-[355px]">
                    ...
                    <motion.div
                      ...
                      transition={{ duration: 0.8, delay: 0.6 + index * 0.1, ease: "easeInOut" }}
                      style={{ originX: 0 }} 
              ...
              <div className="flex items-start gap-3 group cursor-pointer w-fit mt-10">
                <input required type="checkbox" id="terms" className="appearance-none mt-1 min-w-[16px] h-4 border border-[#280F0B]/50 bg-transparent rounded-sm checked:bg-[#280F0B] cursor-pointer relative after:content-[''] after:absolute after:hidden checked:after:block after:left-[4px] after:top-[1px] after:w-[5px] after:h-[9px] after:border-white after:border-r-2 after:border-b-2 after:rotate-45" />
                <label htmlFor="terms" className="font-manrope text-sm text-[#280F0B]/70 cursor-pointer leading-tight">I declare that I have read the terms & conditions.</label>
              </div>

              <button
                disabled={loading}
                type="submit"
                className="w-full lg:w-[355px] py-3 mt-4 border border-[#280F0B] text-[#280F0B] font-manrope font-semibold uppercase tracking-widest text-sm hover:bg-[#280F0B] hover:text-white transition-all disabled:opacity-50"
              >
        ...
        >
          <div className="relative w-full max-w-[450px] aspect-square">
            <Image src="/assets/images/Reg_logo.svg" alt="Mandala" fill className="object-contain" />
          </div>
        </motion.div>
      </div>
    </>
